package com.onethefull.dasomiconv.utils.robot.kebbi
...
object KebbiRobotCommand {
    ...
    fun noJamAnimation(): Job? {
        if (App.instance.currentActivity == null) {
            return (App.instance.currentActivity as MainActivity).lifecycleScope.launch {
                try {
                    delay(5_000)
                    BaseRobotController.robotService?.let { r ->
                        while (true) {
                            r.setMotor(makeRobotCommand(3, -35, 100))
                            r.setMotor(makeRobotCommand(4, -160, 100))
                            r.setMotor(makeRobotCommand(5, -3, 100))
                            r.setMotor(makeRobotCommand(6, -35, 100))
                            delay(2000)
                            r.robotMotor?.down(20)
                            delay(1000)
                            r.robotMotor?.up(20)
                            delay(1000)
                            r.robotMotor?.turnRight(20)
                            delay(1000)
                            r.robotMotor?.turnLeft(20)
                            delay(1000)
                            r.robotMotor.reset()
                            delay(1500)
                            r.setMotor(makeRobotCommand(7, -35, 100))
                            r.setMotor(makeRobotCommand(8, -160, 100))
                            r.setMotor(makeRobotCommand(9, -3, 100))
                            r.setMotor(makeRobotCommand(10, -35, 100))
                            delay(2000)
                            r.robotMotor.reset()
                            delay(1500)
                            checkRunningApp()
    ...
    private suspend fun checkRunningApp() = suspendCoroutine { continuation ->
        if (App.instance.currentActivity == null) {
            DWLog.d("$this job - cancel")
